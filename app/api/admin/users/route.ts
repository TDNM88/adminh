...
export async function GET(req: Request) {
  try {
    ...
    const totalCount = await db.collection<IUser>('users').countDocuments(query);
    ...
    if (users.length === 0 && totalCount === 0 && process.env.NODE_ENV === 'development') {
      ...
      if (mockUsers.length > 0) {
        ...
        const newTotalCount = await db.collection<IUser>('users').countDocuments({});
